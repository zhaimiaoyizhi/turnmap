...
最新预览包：`turnmap-v0.7.1.zip`。这一版重点加固图谱卫生、新生成节点稳定 ID、连接权重、主题代理边元数据、ChatGPT 提取修复，以及调整连接权重后的连接样式稳定性。长期备份或迁移时请优先导出 TurnMap JSON，因为它比视觉格式更完整地保留 TurnMap 专有编辑状态。
...
- `0.7.1`：图谱卫生与自动连接可靠性加固，包括新节点稳定 ID、连接权重、主题代理边元数据和本地修复日志。
...
- **方向与尺寸**：展开时会根据节点相对标题节点的位置选择向左或向右展开；左展开会让父节点向左扩张，右展开会向右扩张。展开区域不做内部滚动，父节点会尽量放大以完整呈现导图。
