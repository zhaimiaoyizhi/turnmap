...
最新预览包：`turnmap-v0.7.1.zip`。这一版重点突出 API 驱动的回答展开 mini 思维导图、mini 导图布局与导出渲染优化，并加固图谱卫生、新生成节点稳定 ID、连接权重、主题代理边元数据、ChatGPT 提取修复，以及连接样式选择、常驻颜色预览等外观自定义内容。长期备份或迁移时请优先导出 TurnMap JSON，因为它比视觉格式更完整地保留 TurnMap 专有编辑状态。
